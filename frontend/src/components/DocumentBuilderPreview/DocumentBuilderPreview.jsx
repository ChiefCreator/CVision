...
import DocumentPreviewNavigation from "./DocumentPreviewNavigation/DocumentPreviewNavigation";

import styles from "./DocumentBuilderPreview.module.scss";

import jsPDF from "jspdf";
import html2canvas from "html2canvas";

export default function DocumentBuilderPreview({ data, isDataLoaded, changeDocumentField }) {
  const [currentPageIndex, setCurrentPageIndex] = useState(0);
  const [totalPages, setTotalPages] = useState(1);

  function downLoadPDFDocument() {
    const content = document.querySelector(`[data-load="${data.id}"]`);
  ...
  return (
    <div className={styles.documentBuilderPreview}>
      <div className={styles.documentBuilderPreviewContainer}>
        <div className={styles.documentBuilderPreviewControls}>
          <Button
          ...
          <ValueAdjuster
            ...
            step={25}
            unitOfMeasurement="%"
            setValue={(value) => changeDocumentField("lineSpacing", value)}
          />
        ...
        <div className={styles.documentBuilderPreviewWrapperPreview}>
          <DocumentPreview
            data={data}
        ...
        <div className={styles.documentBuilderPreviewBottomControls}>
          <DocumentPreviewNavigation currentPageIndex={currentPageIndex} setCurrentPageIndex={setCurrentPageIndex} totalPages={totalPages} />
        </div>
      </div>
    </div>
  );
