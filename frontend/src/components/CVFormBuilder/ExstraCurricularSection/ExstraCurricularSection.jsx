import styles from "./ExstraCurricularSection.module.scss";
...
export default function ExstraCurricularSection({ resumeId, isResumeDataLoaded, data }) {
  const { dispatchOfResumesDataState } = useResumeContext();
  const sectionId = "exstraCurricular";
      ...
      <div className={styles.sectionBody}>
        <div className={styles.sectionFormGroups}>
          <FormGroup gridTemplateColumns="1fr 1fr" gridTemplateRows="1fr 1fr 1fr">
            <FormGroupCell>
              <FormField label="Деятельность" inputValue={data?.activity} inputPlaceholder="Введите деятельность" onChangeInputCallback={(value) => handleFormFieldInputChange("activity", value)} />
            </FormGroupCell>
            <FormGroupCell>
              <FormField label="Работодатель" inputValue={data?.employer} inputPlaceholder="Введите работодателя" onChangeInputCallback={(value) => handleFormFieldInputChange("employer", value)} />
            </FormGroupCell>
            ...
            <FormGroupCell>
              <FormField label="Город" inputValue={data?.city} inputPlaceholder="Введите город" onChangeInputCallback={(value) => handleFormFieldInputChange("city", value)} />
            </FormGroupCell>
          </FormGroup>
        </div>
        <div className={styles.sectionDescriptionWrapper}>
          <FormFieldText label="Описание" placeholder="Опишите ваши обязанности и достижения" isContentLoaded={isResumeDataLoaded} onChangeCallback={(value) => handleFormFieldInputChange("description", value)} value={data?.description}></FormFieldText>
        </div>
      </div>
    </div>
  );
