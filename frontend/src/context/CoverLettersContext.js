...
export const CoverLettersProvider = ({ children }) => {
  ...
  async function getCoverLettersFromDatabase() {
    try {
      const data = await getAllCoverLetters(userData?.uid);
  ...
  async function addCoverLetterToDatabase(data) {
    addCoverLetter(userData?.uid, data);
  };
  async function deleteCoverLetterFromDatabase(id) {
    deleteCoverLetter(userData?.uid, id);
  }
  ...
  useEffect(() => {
    ...
    changedCoverLetter && debounceUpdateCoverLetter(userData?.uid, changedCoverLetter)
