...
export const ResumeProvider = ({ children }) => {
  ...
  async function getResumesFromDatabase() {
    try {
      const data = await getAllResumes(userData.email);
  ...
  async function addResumeToDatabase(resume) {
    await addResume(userData?.email, resume);
  };
  async function deleteResumeFromDatabase(id) {
    deleteResume(userData?.email, id);
  }
  ...
  useEffect(() => {
    ...
    changedResume && debounceUpdateResume(userData.email, changedResume)
