...
export default function Template_1({ resumeData, mainColor, lineHeight }) {
  const personalInformation = resumeData.sections.find(section => section.id === "personalInformation");
  const asideSections = resumeData.sections.filter(section => asideSectionsIds.includes(section.id)).sort((a, b) => (a.order ?? Infinity) - (b.order ?? Infinity));
  const mainSections = resumeData.sections.filter(section => mainSectionsIds.includes(section.id)).sort((a, b) => (a.order ?? Infinity) - (b.order ?? Infinity));
          ...
          <div className={styles.personalInformationContent}>
            ...
            <div className={styles.personalInformationOtherBlock}>
              <span className={styles.personalInformationLocation}>
                {personalInformation.adress && `${personalInformation.adress}`}
